...
func _button_down_furniture(source):
	$FurnitureQueue.add_child(get_node(source).duplicate())
	$Panel.visible = true
	set_physics_process(true)
	snap_furnitures_to_floor()
	get_node(source).visible = false
...
func _on_edit_toggled(toggled_on: bool) -> void:
	...
	if toggled_on == true:
		...
		for i in $FurnitureContainer.get_children():
			...
			i.get_child(3).button_down.connect(_button_down_furniture.bind(i.get_path()))
